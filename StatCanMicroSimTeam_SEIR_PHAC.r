...
SEIR.n.Age.Classes <- function( time=NULL, age.parms = NULL, age.age.parms = NULL,list.inits = NULL, not.parms=  c("tmin", "tmax", "agegrp", "cagegrp", "ragegrp", "isim"))
{
  ...
  calculate_derivatives <- function(time, vec.inits, list.parms, names.inits) {
    ...
    with(as.list(c(list.inits, list.parms)),{
      ...
      betaS <- C_OneMinusLambda <- CqLambda <-  CrOneMinusLambda <- one <- as.matrix(rep(1, nage))
      OneMinusLambda <- (one - lambda)
      
      betaS             <- beta %*% S
      C_OneMinusLambda  <- c_   %*% (one-lambda)
      CqLambda          <- cq   %*% lambda
      CrOneMinusLambda  <- cr   %*% OneMinusLambda
      
      # rates of change that depends on matrices
      #--------------------------------------------
      dS   <- -((C_OneMinusLambda * tau) + CqLambda + (CrOneMinusLambda * (one -  tau))) * betaS * I_sum
      dL   <-  C_OneMinusLambda * tau * betaS * I_sum - sigma * L
      dL_q <-  CqLambda  * betaS * I_sum - sigma * L_q
      dL_r <-  CrOneMinusLambda * (one -  tau) * betaS * I_sum - sigma * L_r
      ...
      dI_ss     <- I_a * delta * epsilon * OneMinusLambda - kappa * I_ss
      ...
      dI_ssisn  <- kappa * (one - feisi-feish) * (I_ss + I_ssr) - I_ssisn * ((one -  mu) * nus + mu * nud)
...
for(i in seq(1, nSim, 1)){
  ...
  out<-out %>%
    mutate(N_tot = rowSums(.[2:111])) # Total number of individuals (=1 Million)
