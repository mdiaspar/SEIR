package_names <- c("janitor","readxl","dplyr","deSolve","tidyr","ggplot2", "ggpubr", "tidyverse", "viridis", "shinycssloaders", "DT", "scales") 
...
ui <- fluidPage(
    ...
    fluidRow(column(12, fileInput("file", "Please upload your model parameters (Excel file)", width = "500px"))),
    tabsetPanel(
        tabPanel("Plots", br(), plotOutput("plot") %>% withSpinner(color = "#337ab7")),
...
server <- function(input, output) {
    ...
    run_model <- reactive({
        ...
        if(is.null(file_to_read)) {
            return(list(big_out = data.frame(), nagegrp = NULL))
        } else {
    ...
    output$table = renderDT(
        run_model()$big_out %>% select(-1) %>% round(),
