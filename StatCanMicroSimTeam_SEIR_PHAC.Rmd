...
SEIR.n.Age.Classes <- function( time=NULL, age.parms = NULL, age.age.parms = NULL,list.inits = NULL, not.parms=  c("tmin", "tmax", "agegrp", "cagegrp", "ragegrp", "isim"))
{
  ...
  calculate_derivatives <- function(time, vec.inits, list.parms, names.inits) {
    ...
    with(as.list(c(list.inits, list.parms)),{
      ...
      #--------------------------------------------
      dS   <- -(BetaC_OneMinusLambda * tau + BetaCqLambda + BetaCrOneMinusLambda * (one -  tau) ) * S * I_sum  
      ...
      dI_aq     <- sigma * rho * L_q - I_aq * delta * epsilonq - I_aq * (one -  delta) * upsilon 
      ...
      dI_ss     <- (I_a + I_aqn) * delta * epsilon * (one - alpha)    - kappa * I_ss  
      dI_smr    <- I_ar * delta * epsilon * alpha - kappa * I_smr
      dI_ssr    <- I_ar * delta * epsilon * (one - alpha)    - kappa * I_ssr 
      ...
      dI_ssh    <- kappa * feish * (I_ss + I_ssr) + delta * (one-alpha) * epsilonq * I_aq - I_ssh * ((one - mu) * nus + mu * nud) 
